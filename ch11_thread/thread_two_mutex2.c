...
/*
 * two mutex simplified version
 * mainly use hashlock to find/hold/release the foo object
 * leaving the f_lock of foo struct only for initialization
 */
...
void foo_hold(struct foo *fp) {
	...
	/*
	 * here prevent other threads from finding/releasing
	 * objects
	 */
	pthread_mutex_lock(&hashlock);
	++fp->f_count;
...
struct foo *foo_find(struct foo *ifp, int id) {
	...
	for(fp = fh[idx]; fp != NULL; fp = fp->f_next) {
		if(fp->f_id == id) {
			/*
			 * already get the lock, no need to call foo_hold,
			 * otherwise, it gets dead lock
			 */
			++fp->f_count;
